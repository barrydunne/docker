...
internal class CreateJobCommandValidatorTests
{
    ...
    [Test]
    public async Task CreateJobCommandValidator_fails_for_missing_idempotency_key()
    {
    ...
    [Test]
    public async Task CreateJobCommandValidator_returns_message_for_missing_idempotency_key()
    {
        var command = _fixture.Build<CreateJobCommand>()
                              .With(_ => _.IdempotencyKey, string.Empty)
                              .Create();
        var result = await _context.Sut.TestValidateAsync(command);
        var error = result.Errors.SingleOrDefault(_ => _.PropertyName == nameof(command.IdempotencyKey) && _.ErrorMessage == "'Idempotency Key' must not be empty.");
