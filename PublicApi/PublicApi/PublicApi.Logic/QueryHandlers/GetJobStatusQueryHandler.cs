...
namespace PublicApi.Logic.QueryHandlers
{
    /// <summary>
    ...
    internal class GetJobStatusQueryHandler : IQueryHandler<GetJobStatusQuery, Job?>
    {
        ...
        /// <inheritdoc/>
        public async Task<Job?> Handle(GetJobStatusQuery query, CancellationToken cancellationToken)
        {
            ...
            try
            {
                ...
                if (job is null)
                {
                    job = await _jobRepository.GetJobIdByIdAsync(query.JobId, cancellationToken);
