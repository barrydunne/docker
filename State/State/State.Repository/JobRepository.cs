...
namespace State.Repository
{
    ...
    public class JobRepository : IJobRepository
    {
        ...
        /// <inheritdoc/>
        public async Task<Job?> GetJobIdByIdAsync(Guid jobId, CancellationToken cancellationToken = default)
        {
        ...
        private static async Task VerifyIndexExistsAsync(IMongoCollection<Job> collection, string databaseName, string collectionName, string[] indexKeyNames, string indexPropertyName, bool uniqueIndex, ILogger logger, CancellationToken cancellationToken)
        {
            ...
            if (indexExists)
            {
                logger.LogDebug("Index {DatabaseName}/{CollectionName}[{IndexPropertyName}] exists: {Exists}", databaseName, collectionName, indexPropertyName, indexExists);
                return;
            }
