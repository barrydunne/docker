...
internal class UpdateGeocodingResultCommandHandlerMetrics : IUpdateGeocodingResultCommandHandlerMetrics
{
    ...
    /// <summary>
    /// Initializes a new instance of the <see cref="UpdateGeocodingResultCommandHandlerMetrics"/> class.
    /// </summary>
    /// <param name="meterFactory">The factory to supply the <see cref="Meter"/>.</param>
    public UpdateGeocodingResultCommandHandlerMetrics(IMeterFactory meterFactory)
    {
        var meter = meterFactory.CreateAssemblyMeter();
        var subjectName = nameof(UpdateGeocodingResultCommand).ToLower();
