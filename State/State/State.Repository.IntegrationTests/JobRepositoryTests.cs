...
namespace State.Repository.IntegrationTests
{
    ...
    public class JobRepositoryTests
    {
        ...
        [Test]
        public async Task JobRepository_GetJobIdByIdAsync_returns_job_if_exists()
        {
            var job = _fixture.Create<Job>();
            await _context.Sut.InsertAsync(job);
            var retrieved = await _context.Sut.GetJobIdByIdAsync(job.JobId);
            Assert.That(JsonSerializer.Serialize(retrieved), Is.EqualTo(JsonSerializer.Serialize(job)));
        }

        [Test]
        public async Task JobRepository_GetJobIdByIdAsync_returns_null_if_unknown()
        {
            var job = _fixture.Create<Job>();
            var retrieved = await _context.Sut.GetJobIdByIdAsync(job.JobId);
        ...
        public async Task JobRepository_DeleteJobByIdAsync_deletes_job()
        {
            ...
            job = await _context.Sut.GetJobIdByIdAsync(job.JobId);
