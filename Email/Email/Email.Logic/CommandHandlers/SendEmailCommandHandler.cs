...
namespace Email.Logic.CommandHandlers
{
    /// <summary>
    ...
    internal class SendEmailCommandHandler : ICommandHandler<SendEmailCommand>
    {
        ...
        public async Task<Result> Handle(SendEmailCommand command, CancellationToken cancellationToken)
        {
            ...
            try
            {
                ...
                await _emailRepository.InsertAsync(new SentEmail { JobId = command.JobId, RecipientEmail = command.Email, SentUtc = DateTime.UtcNow }, cancellationToken);

                return result ? Result.Success() : Result.Failure("Failed to send email");
            }
            catch (Exception ex)
            {
        ...
        private Result PerformGuardChecks(SendEmailCommand command)
        {
            try
            {
                Guard.Against.NullOrEmpty(command.JobId, nameof(SendEmailCommand.JobId));
                Guard.Against.Null(command.Directions, nameof(SendEmailCommand.Directions));
                Guard.Against.Null(command.Weather, nameof(SendEmailCommand.Weather));
                Guard.Against.Null(command.Imaging, nameof(SendEmailCommand.Imaging));
